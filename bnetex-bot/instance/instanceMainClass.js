...
const { getMaxIntervals, getBBRulesIndexes, getBBRuleByIddex, getRulesMoreIntervals } = require("./utils/BollZoneRules");
...
module.exports = class InstanceClass {
  ...
  async klinesHandler(klines){
    ...
    for (let index = 0; index < intervals.length; index++) {
      ...
      if(this.candlesticks_15m[this.candlesticks_15m.length-1].startTime != this.lastHour){
        calculateHour = true;
        this.lastHour = this.candlesticks_15m[this.candlesticks_15m.length-1].startTime;
      }
        ...
        const rule = getBBRuleByIddex(element);
          ...
          if(rule.intervals > 1440){
            ...
            if(calculateHour){
              ...
              candlesticks_15m = CalcBollingerComplex(rule, candlesticks_15m, true); 
            
            
              // Зоны
              candlesticks_15m = CalcZonesByBB(rule, candlesticks_15m, candlesticks[candlesticks.length-1].close, true);
  ...
  async ProbabilityAction(){
    ...
    if(signalToOpen.signal){
      ...
      //console.log('Pattern is opened. Looking for first green kline');
    }
    ...
    //  if(true){
      ...
      await buySignalHandler(this.waitingForFirstGreen.pattern, this.pair, lastKline);
    }
    ...
    await sendMessageToMainChanel(`Покупка по паттерну №${PATTERN__SIGNAL.PATTERN__INDEX}`)
  ...
  async initializing(startTicker = true) {
    ...
    if(this.candlesticks == undefined || !this.candlesticks){           
      ...
      candlesticks_15m = await getPairDataFromExchange(this.pair, '1h', maxInterval/60, false, candlesticks[candlesticks.length-1].startTime);
    ...
    for (let index = 0; index < zoneRulesBB.length; index++) {
      ...
      if(rule.intervals > 1440){
        ...
        // Зоны
        candlesticks_15m = CalcZonesByBB(rule, candlesticks_15m, candlesticks[candlesticks.length-1].close);
    ...
    if(startTicker) {this.startTicker()};
  ...
  startTicker(){
    ...
    let ticketClassSimulate = new TickerClassSimulate(this.pair, this.tradingTimeframes);
      ...
      ticketClassSimulate.initializingSubscribtion().subscribe(async e => {
        ...
        if(e['1m'].startTime % 3600000 == 0){
            ...
            if(this.startHourDate === undefined){
              ...
              let bdate = new Date();
              console.log('HourDate: '.red, (bdate-this.startHourDate) / 1000)
              this.startHourDate = new Date();
            }
        }     
