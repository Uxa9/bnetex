...
module.exports = class AnalyzeModule {
  ...
  async analyze() {
    ...
    if (this.POSITION) {
      WeekMatchingFiltered = this.WeekMatchingList.filter(
        (i) =>
          i.MACRO_WEEK_SITUATION_INDEX == MACRO_WEEK_SITUATION_INDEX &&
    ...
    if (
      ...
      await this._deactivatePatterntByExcludedLogicalGroup(
    ...
    let patternsGroupInLogicalGroup =
      await this._getPatternGroupsByLogicalGroup(
        this.ActiveWeekMatching.LOGICAL_GROUP,
        true
      );
    ...
    if (patternsTempIds.length == 0) {
      ...
      await this._activateLocalGroups(
        patternsTempIds.map((i) => i.id).filter((i) => !i.status)
      );
      await this._getPatternGroupsByLogicalGroup(
        this.ActiveWeekMatching.LOGICAL_GROUP,
        true
    ...
    if (this.POSITION) {
      maxPatternByVolume = patternsTempIds.sort(
        (a, b) => b.PERCENT_OF_DEPOSIT - a.PERCENT_OF_DEPOSIT
      )[0];
    } else {
      maxPatternByVolume = patternsTempIds.sort(
    ...
    if (this.POSITION) {
      activePatternToForce = this.ActivePatternsInWorkingGroup.filter(
        (i) => i.status
      ).sort((a, b) => b.PART_OF_VOLUME - a.PART_OF_VOLUME)[0];
    } else {
      activePatternToForce = this.ActivePatternsInWorkingGroup.filter(
        (i) => i.status
  ...
  async _deactivatePatterntByExcludedLogicalGroup(LOGICAL_GROUP) {
    return await db.models.Pattern.update(
      { status: false },
      {
        //logging: true,
        where: {
